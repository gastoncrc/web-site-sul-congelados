...
function MainLayout() {
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col font-sans text-gray-800 overflow-x-hidden">
      ...
      {/* HEADER RESPONSIVO CONTROLADO */}
      <header className="bg-[#003366] text-white shadow-md sticky top-0 z-50 w-full">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 h-20 flex items-center justify-between relative">
          
          {/* LOGO: Tamaño fluido según pantalla */}
          <div 
            className="absolute top-[-8px] sm:top-[-12px] left-4 sm:left-6 z-50 cursor-pointer drop-shadow-lg transition-transform hover:scale-105" 
          ...
          <nav className="flex items-center space-x-3 sm:space-x-6 md:space-x-8 font-medium text-xs sm:text-sm md:text-base">
            ...
            {user?.role === 'Admin' && (
              <button onClick={() => setCurrentTab('admin')} className="flex items-center space-x-1 text-yellow-400 font-bold hover:text-yellow-300">
                <Shield size={14} /> <span className="hidden md:inline">Panel Admin</span>
              </button>
            )}
          </nav>
          ...
          <div className="flex items-center ml-2 sm:ml-4">
            {user ? (
              <div className="flex items-center space-x-1 sm:space-x-3 bg-[#002244] px-2 py-1 sm:px-4 sm:py-2 rounded-full border border-blue-800 max-w-[120px] sm:max-w-none">
                ...
                <span className="text-[10px] sm:text-xs font-semibold truncate">{user.email.split('@')[0]}</span>
                <button onClick={logout} className="text-red-400 hover:text-red-300 ml-1 sm:ml-2"><LogOut size={14} /></button>
              </div>
            ) : (
              <button onClick={() => setShowLoginModal(true)} className="bg-white text-[#003366] font-bold px-3 py-1.5 sm:px-5 sm:py-2 rounded-lg hover:bg-blue-50 transition shadow text-xs sm:text-sm whitespace-nowrap">
                Acceso Clientes
              </button>
            )}
          </div>
      ...
      <main className="flex-grow max-w-7xl w-full mx-auto px-4 sm:px-6 py-6 sm:py-10">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 sm:gap-8">
            <div className="lg:col-span-2 space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                {products.length > 0 ? (
                  products.map(p => (
                    <div key={p.sku} className="bg-white rounded-xl border border-gray-200 p-4 sm:p-5 shadow-sm flex flex-col justify-between">
                      <div>
                        ...
                        <p className="text-[11px] text-gray-400 mt-2 font-mono">SKU: {p.sku} | Stock: {p.stock} bultos</p>
                      </div>
                      <div className="mt-4 pt-3 border-t border-gray-100 flex items-center justify-between">
                        <div>
                          <span className="text-[10px] text-gray-400 block font-medium">Precio B2B</span>
                  ...
                  <div className="bg-white border p-6 rounded-xl text-center col-span-1 md:col-span-2">
            ...
            <div className="bg-white rounded-xl border border-gray-200 p-4 sm:p-6 shadow-sm h-fit lg:sticky lg:top-24">
              ...
              {cart.length === 0 ? (
                ...
                <div className="space-y-4">
                  {cart.map(item => (
                    <div key={item.product.sku} className="flex justify-between items-center text-sm pb-3 border-b border-gray-100">
                      <div className="pr-2">
                        ...
                        <p className="text-[11px] text-gray-500">{item.quantity} x ${item.product.unitPrice.toLocaleString('es-AR')}</p>
                      </div>
                  ...
                  <button onClick={() => alert('Pedido enviado con éxito. Impactará en el sistema logístico de SUL.')} className="w-full bg-[#003366] text-white py-3 rounded-lg font-bold hover:bg-blue-800 transition mt-4 text-sm">
                    Confirmar Orden B2B
                  </button>
                </div>
              )}
        ...
        {currentTab === 'admin' && user?.role === 'Admin' && (
          <div className="bg-white rounded-xl border border-gray-200 p-4 sm:p-8 shadow-sm max-w-2xl mx-auto">
            <h2 className="text-lg sm:text-xl font-black text-[#003366] mb-6 flex items-center space-x-2">
              <Upload size={20} /> <span>Actualización Masiva de Listas</span>
            </h2>
            <form onSubmit={handleCsvUpload} className="space-y-6">
              <div className="border-2 border-dashed border-gray-300 rounded-lg p-4 sm:p-6 text-center hover:border-blue-500 transition">
                ...
                <label htmlFor="csv-file-input" className="cursor-pointer space-y-2 block">
                  ...
                  <p className="text-xs sm:text-sm font-semibold text-gray-700">{csvFile ? csvFile.name : 'Seleccionar archivo .csv'}</p>
                  <p className="text-[11px] text-gray-400">Columnas: SKU, Minorista, Mayorista, Distribuidor</p>
                </label>
              </div>
              <button type="submit" disabled={!csvFile} className="w-full bg-blue-600 text-white py-3 rounded-lg font-bold hover:bg-blue-700 transition disabled:opacity-50 text-sm">
                Procesar e Inyectar Precios
              </button>
            </form>
          </div>
        )}
        ...
        {currentTab === 'nosotros' && (
          <div className="bg-white rounded-xl border border-gray-200 p-6 sm:p-8 shadow-sm max-w-3xl mx-auto space-y-4">
            <h2 className="text-xl sm:text-2xl font-black text-[#003366]">SUL ALIMENTOS & CONGELADOS</h2>
            <p className="text-sm sm:text-base text-gray-600 leading-relaxed">Somos una empresa líder especializada en el procesamiento mayorista y distribución de soluciones gastronómicas congeladas. Con foco en la eficiencia operativa, garantizamos el abastecimiento continuo a distribuidores, cadenas de fast-food y comedores industriales de todo el país.</p>
          </div>
        )}
        ...
        {currentTab === 'contacto' && (
          <div className="bg-white rounded-xl border border-gray-200 p-6 sm:p-8 shadow-sm max-w-3xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-6 sm:gap-8">
            <div className="space-y-4">
              ...
              <div className="flex items-center space-x-3 text-xs sm:text-sm text-gray-600"><Mail size={16} /> <span>contacto@sul.com</span></div>
            </div>
            <form onSubmit={(e) => { e.preventDefault(); alert('Mensaje comercial recibido.'); }} className="space-y-4">
              <input type="text" placeholder="Razón Social / Empresa" className="w-full p-3 border rounded-lg text-xs sm:text-sm" required />
              <input type="email" placeholder="Correo electrónico" className="w-full p-3 border rounded-lg text-xs sm:text-sm" required />
              <textarea placeholder="Consulta sobre volúmenes o logística" rows={4} className="w-full p-3 border rounded-lg text-xs sm:text-sm" required></textarea>
              <button type="submit" className="w-full bg-[#003366] text-white py-3 rounded-lg font-bold text-xs sm:text-sm hover:bg-blue-800 transition">Enviar Solicitud de Alta</button>
            </form>
          </div>
        )}
      </main>
      ...
      {/* FOOTER */}
      <footer className="bg-gray-900 text-gray-400 py-6 border-t border-gray-800 text-[11px] text-center px-4">
        <p className="font-bold text-gray-200 mb-1">SUL Alimentos & Congelados © 2026</p>
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4 backdrop-blur-sm transition-all">
          <div className="bg-white rounded-2xl p-5 sm:p-8 max-w-md w-full shadow-2xl relative border border-gray-100">
            ...
            <div className="mb-5 pr-6">
              <h3 className="text-lg sm:text-xl font-black text-[#003366] uppercase tracking-wider">Portal Clientes B2B</h3>
              <p className="text-xs text-gray-400 mt-0.5">Ingresá tus credenciales autorizadas de SUL.</p>
            </div>
            ...
            <form onSubmit={handleLoginSubmit} className="space-y-4">
              <div>
                ...
                <input 
                  ...
                  className="w-full p-3 bg-gray-50 border border-gray-200 rounded-lg text-xs sm:text-sm focus:ring-2 focus:ring-blue-600 focus:border-blue-600 outline-none transition-all" 
                  required 
                />
              </div>
              ...
              <button 
                type="submit" 
                className="w-full bg-[#003366] text-white py-3 rounded-lg font-bold text-xs sm:text-sm hover:bg-blue-800 shadow-md hover:shadow-lg transition-all mt-2"
              >
